...
<spawn-write path="new/image/file.png" description="Uploaded image to codebase">
SPAWN_ATTACHMENT_0
</spawn-write>
`;
      messageContent += "\n\n" + generateDump(req);
    ...
    if (
      ...
      messageContent = `
<spawn-write path="src/bad-file.ts" description="Fix 2 errors and introduce a new error.">
...
x.nonExistentMethod2();
    ...
    if (
      ...
      // Fix errors in create-ts-errors.md and introduce a new error
      messageContent = `
<spawn-write path="src/bad-file.ts" description="Fix remaining error.">
// Import doesn't exist
// import NonExistentClass from 'non-existent-class';
...
x.toString(); // replaced with existing method
    ...
    if (
      ...
      messageContent = `
      Fixing the error...
      <spawn-write path="src/pages/Index.tsx">
...
export default Index;
    ...
    if (
      ...
      messageContent = `[[STRING_IS_FINISHED]]";</spawn-write>\nFinished writing file.`;
      messageContent += "\n\n" + generateDump(req);
    }
    const isToolCall = !!(
      lastMessage &&
...
function generateDump(req: Request) {
  ...
  try {
    ...
    return `[[spawn-dump-path=${dumpFilePath}]]`;
  } catch (error) {
