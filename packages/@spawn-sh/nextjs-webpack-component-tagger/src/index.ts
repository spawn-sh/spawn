...
/**
 * A webpack loader that adds `data-spawn-*` attributes to JSX elements.
 */
export default function spawnTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
      ...
      walk(ast as any, {
        enter: (node: any) => {
          try {
            ...
            if (!loc) return;
            const spawnId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-spawn-id="${spawnId}" data-spawn-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[spawn-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[spawn-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[spawn-tagger] ERROR in ${this.resourcePath}:`, err);
