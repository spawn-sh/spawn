(() => {
  const OVERLAY_ID = "__spawn_overlay__";
  let overlay, label;
  ...
  function updateOverlay(el, isSelected = false) {
    ...
    const name = el.dataset.spawnName || "<unknown>";
    const file = (el.dataset.spawnId || "").split(":")[0];
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.spawnId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    window.parent.postMessage(
      {
        type: "spawn-component-selected",
        id: state.element.dataset.spawnId,
        name: state.element.dataset.spawnName,
      },
      "*",
    );
  }
  ...
  function onKeyDown(e) {
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "spawn-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "activate-spawn-component-selector") activate();
    if (e.data.type === "deactivate-spawn-component-selector") deactivate();
  });
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "Spawn component selector initialization failed: document.body not found.",
      );
      return;
    }
    setTimeout(() => {
      if (document.body.querySelector("[data-spawn-id]")) {
        window.parent.postMessage(
          {
            type: "spawn-component-selector-initialized",
          },
          "*",
        );
        console.debug("Spawn component selector initialized");
      } else {
        console.warn(
          "Spawn component selector not initialized because no DOM elements were tagged",
        );
      }
    }, 0);
  }
