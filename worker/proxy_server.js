...
let spawnShimContent = null;
let spawnComponentSelectorClientContent = null;
try {
...
try {
  const spawnShimPath = path.join(__dirname, "spawn-shim.js");
  spawnShimContent = fs.readFileSync(spawnShimPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] spawn-shim.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read spawn-shim.js: ${error.message}`,
  );
}

try {
  const spawnComponentSelectorClientPath = path.join(
    __dirname,
    "spawn-component-selector-client.js",
  );
  spawnComponentSelectorClientContent = fs.readFileSync(
    spawnComponentSelectorClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] spawn-component-selector-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read spawn-component-selector-client.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  // These are strings that were used since the first version of the spawn shim.
  // If the spawn shim is used from legacy apps which came pre-baked with the shim
  ...
  if (!legacyAppWithShim) {
    ...
    if (spawnShimContent) {
      scripts.push(`<script>${spawnShimContent}</script>`);
    } else {
      scripts.push(
        '<script>console.warn("[proxy-worker] spawn shim was not injected.");</script>',
      );
    }
  }
  if (spawnComponentSelectorClientContent) {
    scripts.push(`<script>${spawnComponentSelectorClientContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] spawn component selector client was not injected.");</script>',
    );
  }
