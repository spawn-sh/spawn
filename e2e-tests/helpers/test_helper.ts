...
export class PageObject {
  ...
  async setUpSpawnPro({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    await this.baseSetup();
    await this.goToSettingsTab();
    if (autoApprove) {
      await this.toggleAutoApprove();
    }
    await this.setUpSpawnProvider();
    await this.goToAppsTab();
  }
  ...
  async setUpSpawnProvider() {
    await this.page
      .locator("div")
      .filter({ hasText: /^SpawnNeeds Setup$/ })
      .nth(1)
      .click();
    await this.page.getByRole("textbox", { name: "Set Spawn API Key" }).click();
    await this.page
      .getByRole("textbox", { name: "Set Spawn API Key" })
      .fill("testspawnkey");
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[spawn-dump-path=*]]" is replaced with a placeholder path
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[spawn-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[spawn-dump-path=([^\]]+)\]\]/g, "[[spawn-dump-path=*]]");
  ...
  getChatInput() {
    return this.page.locator(
      '[data-lexical-editor="true"][aria-placeholder="Ask Spawn to build..."]',
    );
  }
  ...
  async setUpAzure({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    ...
    // so we don't need additional setup steps like setUpSpawnProvider
    await this.goToAppsTab();
  }
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "spawn-apps", appName);
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$spawnUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.SPAWN_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.SPAWN_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      ...
      const userDataDir = path.join(baseTmpDir, `spawn-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$spawnUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing spawn.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im spawn.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed spawn.exe");
        } catch (error) {
          console.warn(
            "Failed to kill spawn.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<spawn-file path="package\.json">[\s\S]*?<\/spawn-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
