...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-spawn.tsx");
  ...
  await po.page.getByText("made-with-spawn.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithSpawn = ;");
  ...
  expect(editedFile).toContain("export const MadeWithSpawn = ;");
});
