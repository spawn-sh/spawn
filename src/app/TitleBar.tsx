...
import { SpawnProSuccessDialog } from "@/components/SpawnProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showSpawnProSuccessDialog = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "spawn-pro-return") {
        await refreshSettings();
        showSpawnProSuccessDialog();
      }
    };
    handleDeepLink();
  }, [lastDeepLink]);
  ...
  const isSpawnPro = !!settings?.providerSettings?.auto?.apiKey?.value;
  const isSpawnProEnabled = Boolean(settings?.enableSpawnPro);
  ...
  return (
    <>
      <div className="@container z-11 w-full h-11 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <img src={logo} alt="Spawn Logo" className="w-6 h-6 mr-0.5" />
        <Button
        ...
        {isSpawnPro && <SpawnProButton isSpawnProEnabled={isSpawnProEnabled} />}
...
export function SpawnProButton({
  isSpawnProEnabled,
}: {
  isSpawnProEnabled: boolean;
}) {
  const { navigate } = useRouter();
  const { userBudget } = useUserBudgetInfo();
  return (
    <Button
      data-testid="title-bar-spawn-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isSpawnProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isSpawnProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isSpawnProEnabled && (
