...
export function getSpawnAppPath(appPath: string): string {
  if (IS_TEST_BUILD) {
    const electron = getElectron();
    return path.join(electron!.app.getPath("userData"), "spawn-apps", appPath);
  }
  return path.join(os.homedir(), "spawn-apps", appPath);
}
