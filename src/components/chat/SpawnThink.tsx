...
import { VanillaMarkdownParser } from "./SpawnMarkdownParser";
import { CustomTagState } from "./stateTypes";
import { SpawnTokenSavings } from "./SpawnTokenSavings";

interface SpawnThinkProps {
  node?: any;
  children?: React.ReactNode;
}

export const SpawnThink: React.FC<SpawnThinkProps> = ({ children, node }) => {
  ...
  const tokenSavingsMatch =
    typeof children === "string"
      ? children.match(
          /^spawn-token-savings\?original-tokens=([0-9.]+)&smart-context-tokens=([0-9.]+)$/,
        )
      : null;

  // If it's token savings format, render SpawnTokenSavings component
