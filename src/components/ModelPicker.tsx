import { isSpawnProEnabled, type LargeLanguageModel } from "@/lib/schemas";
...
export function ModelPicker() {
  ...
  const isSmartAutoEnabled =
    settings.enableProSmartFilesContextMode && isSpawnProEnabled(settings);
  ...
  if (settings && isSpawnProEnabled(settings)) {
          ...
          <>
            ...
            {primaryProviders.map(([providerId, models]) => {
              models = models.filter((model) => {
                // Don't show free models if Spawn Pro is enabled because
                // we will use the paid models (in Spawn Pro backend) which
