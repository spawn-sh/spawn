...
export function SetupBanner() {
  ...
  const handleSpawnProSetupClick = () => {
    posthog.capture("setup-flow:ai-provider-setup:spawn:click");
    IpcClient.getInstance().openExternalUrl(
      "https://www.spawn.sh/pro?utm_source=spawn-app&utm_medium=app&utm_campaign=setup-banner",
    );
  };
  ...
  return (
    <>
      <p className="text-xl text-zinc-700 dark:text-zinc-300 p-4">Setup Spawn</p>
          ...
          >
            ...
            <AccordionContent className="px-4 pt-2 pb-4 bg-white dark:bg-zinc-900 border-t border-inherit">
              ...
              <SetupProviderCard
                className="mt-2"
                variant="spawn"
                onClick={handleSpawnProSetupClick}
                tabIndex={isNodeSetupComplete ? 0 : -1}
                leadingIcon={
                  <img src={logo} alt="Spawn Logo" className="w-6 h-6 mr-0.5" />
                }
                title="Setup Spawn Pro"
                subtitle={
                  <>
    ...
    <div className="mt-3 p-3 bg-(--background-lighter) border rounded-lg text-sm">
      <p>
        ...
        <a
          onClick={() => {
            IpcClient.getInstance().openExternalUrl(
              "https://www.spawn.sh/docs/help/nodejs",
            );
          }}
    ...
    case "finished-checking":
      return (
        <div className="mt-3 text-sm text-red-600 dark:text-red-400">
          Node.js not detected. Closing and re-opening Spawn usually fixes this.
        </div>
      );
    default:
