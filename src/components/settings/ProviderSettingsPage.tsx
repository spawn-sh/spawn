...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  // Use fetched data (or defaults for Spawn)
  const providerDisplayName = isSpawn
    ? "Spawn"
    : (providerData?.name ?? "Unknown Provider");
  const providerWebsiteUrl = isSpawn
    ? "https://academy.spawn.sh/settings"
    : providerData?.websiteUrl;
  const hasFreeTier = isSpawn ? false : providerData?.hasFreeTier;
  const envVarName = isSpawn ? undefined : providerData?.envVarName;
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      if (isSpawn) {
        settingsUpdate.enableSpawnPro = true;
      }
  ...
  // --- Toggle Spawn Pro Handler ---
  const handleToggleSpawnPro = async (enabled: boolean) => {
    setIsSaving(true);
    try {
      await updateSettings({
        enableSpawnPro: enabled,
      });
    } catch (error: any) {
      showError(`Error toggling Spawn Pro: ${error}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen px-8 py-4">
      <div className="max-w-4xl mx-auto">
        ...
        {isSpawn && !settingsLoading && (
          <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
            <div>
              <h3 className="font-medium">Enable Spawn Pro</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Toggle to enable Spawn Pro
              </p>
            </div>
            <Switch
              checked={settings?.enableSpawnPro}
              onCheckedChange={handleToggleSpawnPro}
              disabled={isSaving}
            />
          </div>
        )}
