...
import { hasSpawnProKey, type UserSettings } from "@/lib/schemas";
...
export function ProModeSelector() {
  ...
  const toggleProEnabled = () => {
    updateSettings({
      enableSpawnPro: !settings?.enableSpawnPro,
    });
  };

  const hasProKey = settings ? hasSpawnProKey(settings) : false;
  const proModeTogglable = hasProKey && Boolean(settings?.enableSpawnPro);
  ...
  return (
    <Popover>
      <Tooltip>
        ...
        <TooltipContent>Configure Spawn Pro settings</TooltipContent>
      </Tooltip>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <h4 className="font-medium flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-primary font-medium">Spawn Pro</span>
            </h4>
          ...
          <div className="flex flex-col gap-5">
            <SelectorRow
              id="pro-enabled"
              label="Enable Spawn Pro"
              description="Use Spawn Pro AI credits"
              tooltip="Uses Spawn Pro AI credits for the main AI model and Pro modes."
              isTogglable={hasProKey}
              settingEnabled={Boolean(settings?.enableSpawnPro)}
