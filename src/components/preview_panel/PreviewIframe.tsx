...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
    ...
    if (!selectedComponentPreview) {
      if (iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          { type: "deactivate-spawn-component-selector" },
          "*",
        );
      }
      setIsPicking(false);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "spawn-component-selector-initialized") {
        setIsComponentSelectorInitialized(true);
        return;
      }

      if (event.data?.type === "spawn-component-selected") {
  ...
  const handleActivateComponentSelector = () => {
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-spawn-component-selector"
            : "deactivate-spawn-component-selector",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (
    !data ||
    data.type !== "spawn-component-selected" ||
