...
import { createSpawnEngine } from "./llm_engine_provider";
...
const spawnEngineUrl = process.env.SPAWN_ENGINE_URL;
const spawnGatewayUrl = process.env.SPAWN_GATEWAY_URL;
...
export async function getModelClient(
  ...
  const spawnApiKey = settings.providerSettings?.auto?.apiKey?.value;
  ...
  // Handle Spawn Pro override
  if (spawnApiKey && settings.enableSpawnPro) {
    // Check if the selected provider supports Spawn Pro (has a gateway prefix) OR
    // we're using local engine.
    // IMPORTANT: some providers like OpenAI have an empty string gateway prefix,
    // so we do a nullish and not a truthy check here.
    if (providerConfig.gatewayPrefix != null || spawnEngineUrl) {
      ...
      const provider = isEngineEnabled
        ? createSpawnEngine({
            apiKey: spawnApiKey,
            baseURL: spawnEngineUrl ?? "https://engine.spawn.sh/v1",
        ...
        : createOpenAICompatible({
            name: "spawn-gateway",
            apiKey: spawnApiKey,
            baseURL: spawnGatewayUrl ?? "https://llm-gateway.spawn.sh/v1",
          });

      logger.info(
        `\x1b[1;97;44m Using Spawn Pro API key for model: ${model.name}. engine_enabled=${isEngineEnabled} \x1b[0m`,
      );
      if (isEngineEnabled) {
        logger.info(
          `\x1b[1;30;42m Using Spawn Pro engine: ${spawnEngineUrl ?? "<prod>"} \x1b[0m`,
        );
      } else {
        logger.info(
          `\x1b[1;30;43m Using Spawn Pro gateway: ${spawnGatewayUrl ?? "<prod>"} \x1b[0m`,
        );
      }
      ...
      logger.warn(
        `Spawn Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
