...
export async function processFullResponseActions(
  ...
  const appPath = getSpawnAppPath(chatWithApp.app.path);
  ...
  try {
    // Extract all tags
    const spawnWriteTags = getSpawnWriteTags(fullResponse);
    const spawnRenameTags = getSpawnRenameTags(fullResponse);
    const spawnDeletePaths = getSpawnDeleteTags(fullResponse);
    const spawnAddDependencyPackages = getSpawnAddDependencyTags(fullResponse);
    const spawnExecuteSqlQueries = chatWithApp.app.supabaseProjectId
      ? getSpawnExecuteSqlTags(fullResponse)
      : [];
    ...
    if (spawnExecuteSqlQueries.length > 0) {
      for (const query of spawnExecuteSqlQueries) {
        try {
      ...
      logger.log(`Executed ${spawnExecuteSqlQueries.length} SQL queries`);
    }

    // TODO: Handle add dependency tags
    if (spawnAddDependencyPackages.length > 0) {
      try {
        await executeAddDependency({
          packages: spawnAddDependencyPackages,
          message: message,
          appPath,
        });
      } catch (error) {
        errors.push({
          message: `Failed to add dependencies: ${spawnAddDependencyPackages.join(
            ", ",
          )}`,
          error: error,
        });
    ...
    for (const filePath of spawnDeletePaths) {
    ...
    for (const tag of spawnRenameTags) {
    ...
    for (const tag of spawnWriteTags) {
    ...
    hasChanges =
      ...
      spawnAddDependencyPackages.length > 0;
    ...
    if (hasChanges) {
      ...
      if (spawnAddDependencyPackages.length > 0)
        changes.push(
          `added ${spawnAddDependencyPackages.join(", ")} package(s)`,
        );
      if (spawnExecuteSqlQueries.length > 0)
        changes.push(`executed ${spawnExecuteSqlQueries.length} SQL queries`);

      let message = chatSummary
        ? `[spawn] ${chatSummary} - ${changes.join(", ")}`
        : `[spawn] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of Spawn",
            amend: true,
          });
          logger.log(
            `Amend commit with changes outside of spawn: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of Spawn if needed.
          logger.error(
            `Failed to commit changes outside of spawn: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
    ...
    ${warnings
      .map(
        (warning) =>
          `<spawn-output type="warning" message="${warning.message}">${warning.error}</spawn-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<spawn-output type="error" message="${error.message}">${error.error}</spawn-output>`,
      )
      .join("\n")}
    `;
