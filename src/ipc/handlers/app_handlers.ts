...
import { getSpawnAppPath, getUserDataPath } from "../../paths/paths";
...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official Spawn templates) should
      ...
      if (urlMatch) {
        proxyWorker = await startProxy(urlMatch[1], {
          onStarted: (proxyUrl) => {
            safeSend(event.sender, "app:output", {
              type: "stdout",
              message: `[spawn-proxy-server]started=[${proxyUrl}] original=[${urlMatch[1]}]`,
              appId,
            });
          },
        });
...
async function executeAppInDocker({
  ...
  const containerName = `spawn-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.spawn");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.spawn", "-t", `spawn-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `spawn-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (
      ...
      const fullAppPath = getSpawnAppPath(appPath);
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getSpawnAppPath(originalApp.path);
      const newAppPath = getSpawnAppPath(newAppName);
  ...
  handle("get-app", async (_, appId: number): Promise<App> => {
    ...
    // Get app files
    const appPath = getSpawnAppPath(app.path);
    let files: string[] = [];
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getSpawnAppPath("$APP_BASE_PATH"),
    };
  });
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes spawn-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (spawn-pnpm-${appId}).`,
                );
              } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = getSpawnAppPath(app.path);
        const newAppPath = getSpawnAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    const spawnAppPath = getSpawnAppPath(".");
    if (fs.existsSync(spawnAppPath)) {
      await fsPromises.rm(spawnAppPath, { recursive: true, force: true });
      // Recreate the base directory
      await fsPromises.mkdir(spawnAppPath, { recursive: true });
    }
