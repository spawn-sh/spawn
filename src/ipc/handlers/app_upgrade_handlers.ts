...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Spawn component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://spawn.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl: "https://spawn.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes("@spawn-sh/react-vite-component-tagger");
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("spawnComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [spawnComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @spawn-sh/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @spawn-sh/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[spawn] add Spawn component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[spawn] add Capacitor for mobile app support",
    });
