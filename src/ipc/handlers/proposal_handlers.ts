...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getSpawnAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getSpawnChatSummaryTag(messageContent);

        const proposalWriteFiles = getSpawnWriteTags(messageContent);
        const proposalRenameFiles = getSpawnRenameTags(messageContent);
        const proposalDeleteFiles = getSpawnDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getSpawnExecuteSqlTags(messageContent);
        const packagesAdded = getSpawnAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getSpawnWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getSpawnCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getSpawnChatSummaryTag(messageToApprove.content);
