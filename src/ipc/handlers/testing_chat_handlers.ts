...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <spawn-add-dependency packages="deno"></spawn-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <spawn-add-dependency packages="@angular/does-not-exist"></spawn-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <spawn-add-dependency packages="react-router-dom react-query"></spawn-add-dependency>
  
  EOM`,
  write: `Hello world
  <spawn-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[spawn-qa=([^\]]+)\]/);
  if (match) {
