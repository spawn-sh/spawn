...
import { getSpawnAppPath } from "../../paths/paths";
...
const TEMP_DIR = path.join(os.tmpdir(), "spawn-attachments");
...
async function processStreamChunks({
  ...
  for await (const part of fullStream) {
    ...
    if (part.type === "text-delta") {
      ...
      chunk += escapeSpawnTags(part.text);
    } else if (part.type === "tool-call") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeSpawnTags(JSON.stringify(part.input));
      chunk = `<spawn-mcp-tool-call server="${serverName}" tool="${toolName}">\n${content}\n</spawn-mcp-tool-call>\n`;
    } else if (part.type === "tool-result") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeSpawnTags(part.output);
      chunk = `<spawn-mcp-tool-result server="${serverName}" tool="${toolName}">\n${content}\n</spawn-mcp-tool-result>\n`;
    }
      ...
      if (req.attachments && req.attachments.length > 0) {
        ...
        for (const [index, attachment] of req.attachments.entries()) {
          ...
          if (attachment.attachmentType === "upload-to-codebase") {
            ...
            const fileId = `SPAWN_ATTACHMENT_${index}`;
            ...
            // Add instruction for AI to use spawn-write tag
            ...
            if (await isTextFile(filePath)) {
              try {
                attachmentInfo += `<spawn-text-attachment filename="${attachment.name}" type="${attachment.type}" path="${filePath}">
                </spawn-text-attachment>
                \n\n`;
              } catch (err) {
...
`;
      ...
      if (testResponse) {
        ...
        const appPath = getSpawnAppPath(updatedChat.app.path);
        ...
        let systemPrompt = constructSystemPrompt({
          aiRules: await readAiRules(getSpawnAppPath(updatedChat.app.path)),
          chatMode:
        ...
        // print out the spawn-write tags.
...
<spawn-write path="path/to/destination/filename.ext" description="Upload file to codebase">
SPAWN_ATTACHMENT_X
</spawn-write>

Example for file with id of SPAWN_ATTACHMENT_0:
<spawn-write path="src/components/Button.jsx" description="Upload file to codebase">
SPAWN_ATTACHMENT_0
</spawn-write>
...
`;
        ...
        const limitedHistoryChatMessages = limitedMessageHistory.map((msg) => ({
          ...
          content:
            settings.selectedChatMode === "ask"
              ? removeSpawnTags(removeNonEssentialTags(msg.content))
        ...
        const simpleStreamText = async ({
          ...
          spawnDisableFiles = false,
        }: {
          chatMessages: ModelMessage[];
          modelClient: ModelClient;
          tools?: ToolSet;
          systemPromptOverride?: string;
          spawnDisableFiles?: boolean;
        }) => {
          const spawnRequestId = uuidv4();
          if (isEngineEnabled) {
            logger.log(
          ...
          const providerOptions: Record<string, any> = {
            ...
            "spawn-gateway": getExtraProviderOptions(
        ...
        try {
          ...
          if (
            ...
            settings.selectedChatMode !== "ask" &&
            hasUnclosedSpawnWrite(fullResponse)
          ) {
            ...
            while (
              ...
              logger.warn(
                `Received unclosed spawn-write tag, attempting to continue, attempt #${continuationAttempts + 1}`,
              );
          ...
          const addDependencies = getSpawnAddDependencyTags(fullResponse);
          if (
            !abortController.signal.aborted &&
            ...
            try {
              ...
              let problemReport = await generateProblemReport({
                fullResponse,
                appPath: getSpawnAppPath(updatedChat.app.path),
              });
              ...
              while (
                ...
                fullResponse += `<spawn-problem-report summary="${problemReport.problems.length} problems">
${problemReport.problems
  ...
  .join("\n")}
                ...
                const virtualFileSystem = new AsyncVirtualFileSystem(
                  getSpawnAppPath(updatedChat.app.path),
                  {
                    fileExists: (fileName: string) => fileExists(fileName),
                    readFile: (fileName: string) => readFileWithCache(fileName),
                  },
                );
                const writeTags = getSpawnWriteTags(fullResponse);
                const renameTags = getSpawnRenameTags(fullResponse);
                const deletePaths = getSpawnDeleteTags(fullResponse);
...
[Response cancelled by user]`,
      ...
      if (!abortController.signal.aborted && fullResponse) {
        // Scrape from: <spawn-chat-summary>Renaming profile file</spawn-chat-title>
        const chatTitle = fullResponse.match(
          /<spawn-chat-summary>(.*?)<\/spawn-chat-summary>/,
        );
        if (chatTitle) {
          await db
            .update(chats)
  ...
  try {
    if (await isTextFile(filePath)) {
      ...
      const tagPattern = new RegExp(
        `<spawn-text-attachment filename="[^"]*" type="[^"]*" path="${escapedPath}">\\s*<\\/spawn-text-attachment>`,
        "g",
      );
...
export function removeProblemReportTags(text: string): string {
  const problemReportRegex =
    /<spawn-problem-report[^>]*>[\s\S]*?<\/spawn-problem-report>/g;
  return text.replace(problemReportRegex, "").trim();
}

export function removeSpawnTags(text: string): string {
  const spawnRegex = /<spawn-[^>]*>[\s\S]*?<\/spawn-[^>]*>/g;
  return text.replace(spawnRegex, "").trim();
}

export function hasUnclosedSpawnWrite(text: string): boolean {
  // Find the last opening spawn-write tag
  const openRegex = /<spawn-write[^>]*>/g;
  let lastOpenIndex = -1;
  let match;
  ...
  const hasClosingTag = /<\/spawn-write>/.test(textAfterLastOpen);

  return !hasClosingTag;
}

function escapeSpawnTags(text: string): string {
  // Escape spawn tags in reasoning content
  // We are replacing the opening tag with a look-alike character
  // to avoid issues where thinking content includes spawn tags
  // and are mishandled by:
  // 1. FE markdown parser
  // 2. Main process response processor
  return text.replace(/<spawn/g, "＜spawn").replace(/<\/spawn/g, "＜/spawn");
}
