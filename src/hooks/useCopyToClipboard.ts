...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as SpawnMarkdownParser but convert to clean text
      const formattedContent = convertSpawnContentToMarkdown(messageContent);
  ...
  // Convert Spawn content to clean markdown using the same parsing logic as SpawnMarkdownParser
  const convertSpawnContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from SpawnMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from SpawnMarkdownParser)
  ...
  // Reuse the same parsing functions from SpawnMarkdownParser but simplified
