  ...
  try {
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "spawn-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `spawn-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `spawn-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `spawn-${version}.Setup.exe`,
      `spawn-darwin-arm64-${version}.zip`,
      `spawn-darwin-x64-${version}.zip`,
      `spawn_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      "RELEASES",
    ];
